...
private class AikenFindUsagesHandler(element: PsiElement) : FindUsagesHandler(element) {
    ...
    private fun collectTargetFiles(
        ...
        return when (config.scope) {
            ...
            UsageScope.ALL_PROJECT_FILES -> collectProjectFiles(project, config, name)
            UsageScope.IMPORTED_FILES -> {
    ...
    private fun collectRanges(
        ...
        while (lexer.tokenType != null) {
            ...
            if (tokenType == AikenTokenTypes.KEYWORD && config.excludeDeclarationIdentifiers) {
                val word = text.subSequence(lexer.tokenStart, lexer.tokenEnd).toString()
                if (word in setOf("fn", "test", "bench", "validator")) {
                    skipNextIdentifier = true
                }
            }

            if (skipNextIdentifier && tokenType == AikenTokenTypes.IDENTIFIER) {
    ...
    private data class UsageConfig(
        ...
        val excludeDeclarationIdentifiers: Boolean = false
    ) {
        companion object {
            fun fromElement(element: PsiElement, type: com.intellij.psi.tree.IElementType): UsageConfig? =
                when (type) {
                    AikenTokenTypes.FUNCTION -> UsageConfig(
                        lexerFactory = { AikenLexing.createLexer() },
                        tokenTypes = setOf(AikenTokenTypes.FUNCTION, AikenTokenTypes.IDENTIFIER),
                        fileType = AikenFileType,
                        indexId = AikenIdentifierIndex.NAME,
                        scope = UsageScope.IMPORTED_FILES,
                        excludeDeclarationIdentifiers = true
                    )
                    AikenTokenTypes.TYPE -> UsageConfig(
                        lexerFactory = { AikenLexing.createLexer() },
                        tokenTypes = setOf(AikenTokenTypes.TYPE),
                        fileType = AikenFileType,
                        indexId = AikenIdentifierIndex.NAME,
                        scope = UsageScope.IMPORTED_FILES
                    )
                    AikenTokenTypes.IDENTIFIER -> {
                        ...
                        if (isConst) {
                            UsageConfig(
                                lexerFactory = { AikenLexing.createLexer() },
                                tokenTypes = setOf(AikenTokenTypes.IDENTIFIER),
                                fileType = AikenFileType,
                                indexId = AikenIdentifierIndex.NAME,
                                scope = UsageScope.IMPORTED_FILES
                            )
